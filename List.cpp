...
str thaotac[8] = { "QUAN LI THONG TIN KHACH HANG               ","QUAN LI THONG TIN XE                       ", "QUAN LI THONG TIN DANH SACH DON DAT HANG   ", "QUAN LI THONG TIN DANH SACH NHAN VIEN      " , "THONG KE TINH HINH KINH DOANH TRONG NAM    ", "CAP NHAT THONG TIN XE                      ", "DANH SACH KHACH HANG MUA TRA GOP           ", "KET THUC CHUONG TRINH                      "};
																																																			  
str doanhthu[4] = { "DOANH THU THU DUOC TRONG NAM               ","DOANH THU TRUNG BINH TREN MOT KHACH HANG   ", "DOANH THU CAO NHAT VA THAP NHAT TRONG NAM  ", "KET THUC CHUONG TRINH                      " };
str	don_dat_hang[3] = { "QUAN LI THONG TIN DANH SACH DON HANG       ", "QUAN LI THONG TIN DANH SACH LICH HEN       ", "QUAY LAI								   " };
str thao_tac_ban_phim[4] = {"TIM KIEM THONG TIN KHACH HANG              ", "CHINH SUA THONG TIN DANH SACH XE           ", " CHINH SUA THONG TIN DANH SACH NHAN VIEN   ", "QUAY LAI								    " };
																		
str thao_tac_khach[3] = { "TIM KIEM THONG TIN KHACH HANG              ", "XUAT HOA DON CUA KHACH HANG                ", "QUAY LAI								   " };
str thao_tac_xe[4]= { "NHAP THEM XE VAO SHOWROOM OTO              ", "XOA MOT XE BAT KY RA KHOI SHOWROOM OTO     ", "TIM KIEM THONG TIN XE BAT KY TRONG SHOWROOM", "QUAY LAI							 	    "};
str thao_tac_nhan_vien[4]= { "TUYEN THEM NHAN VIEN LAM VIEC              ", "XOA THONG TIN MOT NHAN VIEN BAT KY         ", "TIM KIEM THONG TIN NHAN VIEN TRONG SHOWROOM","QUAY LAI								    " };
...
template<class T>
void List<T>::delete_Head(List<T>& data) {
	Node<T>* p = data.head;
...
template<class T>
void List<T>::delete_Tail(List<T>& data) {
	Node<T>* p = data.tail;
...
template<class T>
void List<T>::delete_AnyWhere(List<T>& data, string nd) {
	Node<T>* p = Tim_kiem(data, nd);
	if (p != nullptr) {
		if (p->prev == nullptr) {
			delete_Head(data);
			return;
...
void List<T>::Danh_sach_nhan_vien(List<NhanVien> a, List<ChucVu> b) {
	for (Node<NhanVien>* p = a.get_Head(); p!= nullptr; p = p->next) {
		cout << p->data.get_MSNV() << setw(20);
		cout << p->data.get_Ten_NV() << setw(30);
		cout << p->data.get_Gioi_tinh() << setw(20);
		cout << p->data.get_Dia_chi() << setw(25);
		for (Node<ChucVu>* k = b.get_Head(); k!= nullptr; k = k->next) {
			if (p->data.get_Ma_chuc_vu() == k->data.get_Ma_chuc_vu()) {
				cout << k->data.get_Chuc_vu() << setw(20);
				cout << k->data.get_Luong_co_ban() << endl;
			}
		}
	}
}
...
template<class T>
ll List<T>::Doanh_thu(List<DonHang> a, List<Xe> b) {
	ll Tong_doanh_thu = 0;
	Node<DonHang>* l;
	Node<Xe>* m;
	for (Node<DonHang>* p = a.get_Head(); p != nullptr; p = p->next) {
...
// Khi làm duoc file don hang thi cho chuc nang cua no trong menu nen cho ham nay vao luon
template<class T>
...
int menu_xe(str thao_tac_xe[4], int n) {
	List<Xe> xe;
	int mn = 1;
	int tt = 0;
	int* mau = new int[n];
...
int menu_thao_tac_ban_phim(str thao_tac_ban_phim[4], int n) {
	List<DonDatHang> dathang;
	List<Xe> xe;
	List<KhachHang> khach;
