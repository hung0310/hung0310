...
str thaotac[9] = { "QUAN LI THONG TIN KHACH HANG               ","QUAN LI THONG TIN XE                       ", "QUAN LI THONG TIN DANH SACH DON DAT HANG   ", "QUAN LI THONG TIN DANH SACH NHAN VIEN      " , "THONG KE TINH HINH KINH DOANH TRONG NAM    ", "TIM KIEM HOA DON KHACH HANG                ", "DANH SACH KHACH HANG MUA TRA GOP           ", "LOI NHUAN THU DUOC TRONG NAM               ", "KET THUC CHUONG TRINH                      "};
...
str thao_tac_ban_phim[4] = {"TIM KIEM THONG TIN KHACH HANG              ", "CHINH SUA THONG TIN DANH SACH XE           ", " CHINH SUA THONG TIN DANH SACH NHAN VIEN   ", "KET THUC CHUC NANG                         " };
																		
str thao_tac_khach[3] = { "TIM KIEM THONG TIN KHACH HANG              ", "XUAT HOA DON CUA KHACH HANG                ", "QUAY LAI								   " };
str thao_tac_xe[4]= { "NHAP THEM XE VAO SHOWROOM OTO              ", "XOA MOT XE BAT KY RA KHOI SHOWROOM OTO     ", "TIM KIEM THONG TIN XE BAT KY TRONG SHOWROOM", "KET THUC CHUC NANG                         " };
str thao_tac_nhan_vien[4]= { "TUYEN THEM NHAN VIEN LAM VIEC              ", "XOA THONG TIN MOT NHAN VIEN BAT KY         ", "TIM KIEM THONG TIN NHAN VIEN TRONG SHOWROOM","KET THUC CHUC NANG                         " };
...
void List<T>::Tim_kiem_hoa_don_khach_hang(List<DonHang> dh, List<KhachHang> kh, List<Xe> x) {
	string  kt;
	cout << "Nhap ma so khach hang co dinh dang la: MSDH_*:.... ";
	getline(cin, kt);
	int dem = 0;
	int count = 0;
	ll tong = 0;
	...
	cout << char(217) << endl;
	if (dem == dh.get_Size()) {
		cout << "MA SO KHONG HOP LE HOAC KHONG TON TAI TRONG DANH SACH" << endl;
	}
}
...
void List<T>::Loi_nhuan_thu_duoc_trong_nam(List<DonHang>a, List<Xe> b, List<NhanVien> c, List<ChucVu> d, List<ChiPhi> e) {
	...
	ll Loi_nhuan = Tong_doanh_thu(a, b) - Tong_chi_phi;
	cout << "|      " << Tong_doanh_thu(a, b) << setw(25);
